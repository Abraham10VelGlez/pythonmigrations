...
for num in range(1, 126):
    ...
    headercsv = ""
    sqlcommand = ""
    with open( cadenasfilescarpt + '/predios.csv', newline='', encoding='latin_1') as csvfile:
        spamreader = csv.reader(csvfile, delimiter=',', quotechar='|')
        for row in spamreader:
            #creas la tabla que contenga los datos
            headercsv ='drop table IF EXISTS '+ cadenaschemasql +'.predios CASCADE; create table '+ cadenaschemasql +'.predios ('+ ' text, '.join(row).lower()+ ' text );'
            sqlcommand ='insert into ' + cadenaschemasql + '.predios ('+ ' , '.join(row).lower()+ '  ) VALUES ( '
    ...
    class load_avg:
        ...
        def load_excel(avg):
            cadena_resultante = cadenasfilescarpt +'/predios.csv'
        ...
        def load_to_bd(avg):
            ...
            avg.cur.execute("truncate " +cadenaschemasql+ ".predios;");
            ...
            avg.connection.commit()
            print("TABLA PREDIOS DEL MUNICIPIO "+ str(num).zfill(3) +" CARGADA A POSTGRESQL ")
