...
print("leer csv header para solo columnas propiedades x municipio")
...
for num in range(1, 126):
    ...
    with open( cadenaschema + '/propiedades.csv', newline='', encoding='latin_1') as csvfile:
        spamreader = csv.reader(csvfile, delimiter=',', quotechar='|')
        for row in spamreader:
            headercsv ='drop table IF EXISTS ' + cadenaschema_base_datos + '.propiedades CASCADE; create table ' + cadenaschema_base_datos + '.propiedades ('+ ' text, '.join(row).lower()+ ' text );'
            sqlcommand ='insert into '+ cadenaschema_base_datos +'.propiedades ('+ ' , '.join(row).lower()+ '  ) VALUES ( '
    ...
    conn.commit()
    print("TABLAS DE PROPIEDADES CREADAS EXITO")
    conn.close()
