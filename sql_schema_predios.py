...
print("leer csv header para solo columnas predios x municipio")
...
for num in range(1, 126):
    ...
    with open( cadenaschema + '/predios.csv', newline='', encoding='latin_1') as csvfile:
        spamreader = csv.reader(csvfile, delimiter=',', quotechar='|')
        for row in spamreader:
            headercsv ='drop table IF EXISTS ' + cadenaschema_base_datos + '.predios CASCADE; create table ' + cadenaschema_base_datos + '.predios ('+ ' text, '.join(row).lower()+ ' text );'
            sqlcommand ='insert into '+ cadenaschema_base_datos +'.predios ('+ ' , '.join(row).lower()+ '  ) VALUES ( '
    ...
    conn.commit()
    print("TABLAS DE PREDIOS CREADAS EXITO")
    conn.close()
